'''
Docstring for Chapter 13.Labs.13.13 LAB Artwork label(classees-constructors)
...
class Artist:
    ...
    def print_info(self):
        if self.birth_year >= 0 and self.death_year >= 0:
            ...
            # Still alive (birth known, death unknown)
...
class Artwork:
    ...
    def __init__(self, title="unknown", year_created=-1, artist=None):
        ...
        # If no artist provided, create default Artist
        if artist is None:
            self.artist = Artist()
        else:
            self.artist = artist
