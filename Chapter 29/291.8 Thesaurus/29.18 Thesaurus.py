...
with open(file_name, 'r') as words:
    for line in words:
        lines = line.split(' ')  # Split by space
        
        for i in range(len(lines)):
            word = lines[i].strip()  # Remove whitespace/newlines
            
            # Get first letter of THIS word
            first_letter = word[0]
            
            # Check if this letter is already a key in dictionary
            if first_letter not in synonyms:
                synonyms[first_letter] = []  # Create empty list
            
            # Add word to the list for this letter
            synonyms[first_letter].append(word)

words.close()

# NOW search for the user's letter
if letter in synonyms:
    for word in synonyms[letter]:
        print(word)
else:
