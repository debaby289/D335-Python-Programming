'''
Suppose a launched toy rocket's height is computed as h = (vi*t) - (5t2). vi is the initial velocity, and t is time in seconds since the launch (starting at 0). Write a program to read the initial velocity, then output the time and the rocket's height (as an integer) once per second. Stop when the height would be negative, meaning the rocket hit the ground (don't print the negative). Assume all values are integers. Ex: For input 20, the output is:
    0 0
    1 15
    2 20
    3 15
...
Hints:
    ...
    Use this expression for the while loop: (rocket_height >= 0).

    Don't forget to increment time inside the while loop (else you'll get an infinite loop).
'''
time = 0 
velocity = int(input())
rocket_height = (velocity * time) - (5 * (time * time))

while rocket_height >= 0:
    print(f'{time} {rocket_height}')
    time += 1
    rocket_height = (velocity * time) - (5 * (time * time))