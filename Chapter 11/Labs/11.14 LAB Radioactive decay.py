'''
Docstring for Chapter 11.Labs.11.14 LAB Radioactive decay
...
def compute_Nt(N0, t, T):
    # Formula: Nt = N0 * e^((-t * ln(2)) / T)
    Nt = N0 * math.exp((-t * math.log(2)) / T)
    return Nt

# Return the half-life of an isotope given that
# a mass N0 decays to Nt after time t
def compute_T(N0, Nt, t):
    # Formula: T = (-t * ln(2)) / ln(Nt / N0)
    T = (-t * math.log(2)) / math.log(Nt / N0)
    return T
